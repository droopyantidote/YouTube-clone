...
const API_KEY = "AIzaSyAwqF8KEMb7kE7dIRUE9dOAWi7t1aM6p1k";
  ...
  items.forEach(async (item) => {
    ...
    videoDiv.innerHTML = `
                <div class="thumbnail">
                    ...
                    <div class="${videoId ? 'duration' : ''}"> ${videoId ? time : ''}</div>
                </div>
                <div class="details">
                    <div style="padding:5px;">
                        ...
                        <p class="${videoId ? 'time' : ''}">${videoId ? view + ' views  &#x2022; ' + date : ''}</p>
                    </div>
                </div>
        `;
...
async function fetchVideoStats(videoId, typeOfDetails) {
    try{
        ...
        if(typeOfDetails==="contentDetails")
          console.log(typeOfDetails+' inside stats func'+ data.items,data.items[0].contentDetails);
        else console.log(typeOfDetails+' inside stats func'+ data.items,data.items[0].statistics);
        return data;
    }
    catch(error){
        console.log(videoId);
...
function formatDuration(duration){//PT1M19S 
  let hour='';
  let min='';
  let sec='';
  ...
  if(hourIdx===-1 && minIdx===-1){// in seconds PT58S
    sec=duration.slice(2,-1);
    if(sec.length==1)sec = '0'+sec;
    return '0:'+sec;
  }
  else if(hourIdx===-1){//in minutes PT2M3S
  ...
  else{//in hours 10:03:45 PT10H3M45S
    hour = duration.slice(2,hourIdx);
    if(minIdx===-1 && secIdx===-1){//PT1H
      return hour+':00'+':00';
    }
    if(secIdx==-1){//PT1H30M
